# Support functions that allow Fivethirtyeight to be scraped. There should only be function definitions in this file, nothing that gets executed

# 1: Returns ~a list of links where we can find links~ to news articles for the last x months
## For this its "https://fivethirtyeight.com/sitemap-index-1.xml", and the links we want are at the tail end of that list of links
fivethirtyeight_sitemap  <- function(last_x_months){
  
  sitemap        <- GET("https://fivethirtyeight.com/sitemap-index-1.xml") %>% read_html %>% html_nodes("loc") %>% html_text
  sitemap_links  <- sitemap # Not sure how the articles are being stored, just collecting everything and filter by date %>% tail(last_x_months*3)
  
}


# 2: Given the results of the last function, visit each sitemap link and get a list of article links
fivethirtyeight_find_links <- function(link){
...
fivethirtyeight_article <- function(link){
  ...
  try({
    ...
    results <- data.table(article_date     = {article %>% html_node(".datetime")  %>% html_text %>% str_trim  %>% str_c(collapse = "\n")},
                          article_headline = {article %>% html_node(".article-title-single")    %>% html_text %>% str_trim %>% str_c(collapse = "\n") %>% first},
                          article_author   = {article %>% html_node(".author")  %>% html_text   %>% str_trim  %>% str_c(collapse = "\n")},
                          article_source   = {"FiveThirtyEight"},
                          article_tag      = {article %>% html_node(".term")  %>% html_text %>% str_trim %>% str_c(collapse = "\n")},
                          article_text     = {article %>% html_node(".single-post-content")  %>% html_text %>% str_trim %>% str_c(collapse = "\n")},
